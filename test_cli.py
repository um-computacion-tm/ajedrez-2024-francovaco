...
class TestCli(unittest.TestCase):
    ...
    def test_exit_from_row(self, mock_input, mock_stdout):
        ...
        La función test_salir verifica que, cuando se simula la entrada del usuario con el valor "SALIR" en lugar de la coordenada de la fila origen, 
    ...
    def test_exit_from_col(self, mock_input, mock_stdout):
        '''
        La función test_salir es una prueba unitaria que verifica cómo se maneja la entrada "SALIR" en el tablero de ajedrez.
        La función test_salir verifica que, cuando se simula la entrada del usuario con el valor "SALIR" en lugar de la coordenada de la columna origen,
        el programa maneja la entrada correctamente sin intentar mover ninguna pieza, muestra el mensaje "Juego terminado." y finaliza el juego.
        '''
        chess = MagicMock()
        with self.assertRaises(SystemExit):
    ...
    def test_exit_to_row(self, mock_input, mock_stdout):
        '''
        La función test_salir es una prueba unitaria que verifica cómo se maneja la entrada "SALIR" en el tablero de ajedrez.
        La función test_salir verifica que, cuando se simula la entrada del usuario con el valor "SALIR" en lugar de la coordenada de la fila destino,
        el programa maneja la entrada correctamente sin intentar mover ninguna pieza, muestra el mensaje "Juego terminado." y finaliza el juego.
        '''
        chess = MagicMock()
        with self.assertRaises(SystemExit):
    ...
    def test_exit_to_col(self, mock_input, mock_stdout):
        '''
        La función test_salir es una prueba unitaria que verifica cómo se maneja la entrada "SALIR" en el tablero de ajedrez.
        La función test_salir verifica que, cuando se simula la entrada del usuario con el valor "SALIR" en lugar de la coordenada de la columna destino,
        el programa maneja la entrada correctamente sin intentar mover ninguna pieza, muestra el mensaje "Juego terminado." y finaliza el juego.
        '''
        chess = MagicMock()
        with self.assertRaises(SystemExit):
    ...
    def test_valid_move_pawn(self, mock_show_board, mock_input, mock_stdout):
        ...
        La función test_movimiento_valido verifica que, cuando se simula la entrada del usuario para mover una pieza de ajedrez desde la posición (fila, columna) 
    ...
    def test_non_numeric_coordinates_from_row(self, mock_show_board, mock_input, mock_stdout):
        ...
        chess.turn = 'white'
        play(chess)
        self.assertIn("Debe ingresar valores numéricos entre 0 y 7.", mock_stdout.getvalue())
    ...
    def test_symbol_coordinates_from_col(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_symbol_coordinates_from_col es una prueba unitaria que verifica cómo se maneja una entrada no numérica inválida en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
    ...
    def test_symbol_coordinates_to_row(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_symbol_coordinates_to_row es una prueba unitaria que verifica cómo se maneja una entrada no numérica inválida en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
    ...
    def test_symbol_coordinates_to_col(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_symbol_coordinates_to_col es una prueba unitaria que verifica cómo se maneja una entrada no numérica inválida en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
    ...
    def test_out_of_range_coordinates_to_row(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_out_of_range_coordinates_to_row es una prueba unitaria que verifica cómo se maneja una entrada fuera del rango válido en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
        play(chess)
        self.assertIn("Los valores de fila y columna deben estar entre 0 y 7.", mock_stdout.getvalue())
    ...
    def test_out_of_range_coordinates_from_row(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_out_of_range_coordinates_from_row es una prueba unitaria que verifica cómo se maneja una entrada fuera del rango válido en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
    ...
    def test_out_of_range_coordinates_from_col(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_out_of_range_coordinates_from_col es una prueba unitaria que verifica cómo se maneja una entrada fuera del rango válido en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
    ...
    def test_out_of_range_coordinates_to_col(self, mock_stdout, mock_show_board, mock_input):
        '''
        La función test_out_of_range_coordinates_to_col es una prueba unitaria que verifica cómo se maneja una entrada fuera del rango válido en el tablero de ajedrez.
        '''
        chess = MagicMock()
        chess.get_board.return_value = [['.']*8 for _ in range(8)]
        chess.turn = 'white'
